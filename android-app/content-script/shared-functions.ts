...
async function getDBCache() {
	...
	if (getDiffInDays(GCdate, date) >= GCdiff) garbageCollection()
}
...
async function startShowRatingInterval(optionShowRating = true) {
	if (showRating()) addRating(optionShowRating)
...
function getAllTitleCardsTypes(): Array<NodeListOf<Element>> {
	...
	if (isDisney) AllTitleCardsTypes = [document.querySelectorAll("a[data-testid='set-item']:not(.imdb)")]
...
async function addRating(showRating: boolean) {
	url = window.location.href
	...
	for (let type = 0; type < AllTitleCardsTypes.length; type++) {
		...
		for (let i = 0; i < titleCards.length; i++) {
			...
			// add seen class
			if (isDisney || isHotstar || isHBO) card.classList.add("imdb")
			else if (isPrimeVideo) {
